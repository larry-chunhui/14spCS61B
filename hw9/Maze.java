...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    if (vert > 1) {
      hWalls = new boolean[horiz][vert - 1];  //horizontal
      for (j = 0; j < vert - 1; j++) {
        for (i = 0; i < horiz; i++) {
          hWalls[i][j] = true;
        }
    ...
    if (horiz > 1) {
      vWalls = new boolean[horiz - 1][vert]; //vertical
      for (i = 0; i < horiz - 1; i++) {
        for (j = 0; j < vert; j++) {
          vWalls[i][j] = true;
        }
    ...
    if(horiz>1){
      for (j = 0; j < vert; j++) {
        for (i = 0; i < horiz-1; i++) {
//          if(i==0 && j==0){
//            allWalls[index]=Integer.MIN_VALUE;
//          }
//          else{
//            allWalls[index]=-(i+(horiz)*j);
//          }
 ...
 //     System.out.println("i= "+i+" "+allWalls[i]+" ");
    }
    System.out.println("Initialization finished");

    int cur=numWalls-1;
    while(cur>0){
      int select= randInt(cur);
      int temp=allWalls[cur];
      allWalls[cur]=allWalls[select];
      allWalls[select]=temp;
      cur--;
    }

    System.out.println("allWalls shuffled= ");

    for(i=0;i<numWalls;i++){
 //     System.out.print(allWalls[i]+" ");
    }
    System.out.println();
    ...
    for(int k=0;k<numWalls;k++){
      index=allWalls[k];
      i=Math.abs(index)%horiz;
      j=Math.abs(index)/horiz;
 //     System.out.println(index+" "+i+" "+j);
      if(index>0){   //horizontal wall
