...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    if(isValidNode()){
      ...
      temp.parent= this;
      if(c<=1){ //important
        temp.nextSibling= this.firstChild;
        this.firstChild=temp;
      }
      else if(c>children()){ //correct
        ((SibTreeNode) this.child(children())).nextSibling=temp;
      }
      else{
        temp.nextSibling=(SibTreeNode) child(c);
        ((SibTreeNode) child(c-1)).nextSibling=temp;
      }
  ...
  public void removeLeaf() throws InvalidNodeException {
    ...
    if(children()<=0){
      myTree.size--;
      if (this==(SibTreeNode) myTree.root()){
        valid = false;
        //this= null;
      }
      else{
        if(this==this.parent.firstChild){
          ...
          SibTreeNode cur= this.parent.firstChild;
