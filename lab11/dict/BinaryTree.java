...
//    return null;
  ...
  public Entry remove(Object key) {
    ...
    if(node==null) {return null;}
      ...
      //the node has two children
        ...
        node.entry=min.entry; //??? can entry it self be chenged to another entry?
        if(min==node.rightChild){
          node.rightChild=null;
          min.parent=null;
        }
        else if(min.rightChild!=null){  //min has right child;
          min.rightChild.parent=min.parent;
          min.parent.leftChild=min.rightChild;
          min.parent=null;
        }
        else{  //min has no child
 ...
 //   System.exit(0);
    ...
    tree.testRemove(1, "(2Z)6V(9L)"); //problem
